...
| `Umbraco.Community.Sanitiser.Forms` | Optional. Deletes [Umbraco Forms](https://umbraco.com/products/umbraco-forms/) submissions and their uploaded files. Install only on sites that use Umbraco Forms. |

The `Faker` and `AI` packages are separate because each brings an extra dependency and changes replacement
behaviour; install one only if you want it. The `Forms` package adds a sanitiser for a specific add-on
(Umbraco Forms). All depend on `Core`, not on the main package.
...
By default all users/members are loaded into memory in one go. On very large sites you can bound this with
`MaxRecords` (per sanitiser); when the total exceeds it, the first `MaxRecords` are processed and a warning is
logged. In `Delete` mode, running again processes the next batch.
...
When Umbraco saves or deletes a member it writes a cache-refresh instruction to the `umbracoCacheInstruction`
table, and that instruction's payload can contain the member's username (including the *previous* username on
rename). So after sanitising, the member/user sanitisers clear pending cache instructions to make sure no
personal data lingers there.

> [!NOTE]
> Umbraco only provides a prune-by-date delete, so this clears **all** pending cache instructions, not just
> member/user ones. On a single server this has no observable effect (local caches are refreshed in-process).
> On a **load-balanced** setup, other nodes will miss any pending cache refreshes and rebuild those caches
> lazily — another reason this package is intended for non-production environments. (Umbraco prunes this table
> itself within ~2 days regardless.)
...
Form submissions are user-entered personal data, so a sanitised copy should not keep them. Install the optional
`Umbraco.Community.Sanitiser.Forms` package on sites that use [Umbraco Forms](https://umbraco.com/products/umbraco-forms/)
and enable it:
...
- **Umbraco Forms submissions** are handled by the optional `Umbraco.Community.Sanitiser.Forms` package (see
  [Umbraco Forms](#umbraco-forms)). Other third-party data stores are out of scope; add a custom `ISanitiser`
  for those.
