...
Sanitiser is split into a small family of packages so you can install only the strategies you need:

| Package | Contents |
|---|---|
| `Umbraco.Community.Sanitiser` | Meta-package — installs Core, Users and Members |
| `Umbraco.Community.Sanitiser.Core` | The `ISanitiser` extension point, orchestration, and the `DatabaseTableSanitiser`/`DirectorySanitiser` base classes |
| `Umbraco.Community.Sanitiser.Users` | Removes/anonymises backoffice users (except the Super Admin) |
...
The `Faker` and `AI` packages are deliberately excluded from the meta-package: each brings an extra dependency
and changes replacement behaviour, so they are opt-in.
...
Or install an individual strategy package (Core comes with it automatically), e.g.:

`dotnet add package Umbraco.Community.Sanitiser.Members`
